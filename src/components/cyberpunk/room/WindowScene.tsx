...
function WindowView({
  ...
  windowMeshRef: React.RefObject<THREE.Mesh | null>;
...
function WindowFrame({
  ...
  const hd = ROOM.depth / 2;
  const frameColor = variant === 'day' ? '#d1d5db' : '#1a1a25';

  return (
    <>
      <mesh position={[windowCenterX, windowCenterY + WINDOW.height / 2, -hd + 0.03]}>
        <boxGeometry args={[WINDOW.width + 0.1, 0.08, 0.08]} />
        <meshStandardMaterial color={frameColor} metalness={variant === 'day' ? 0.35 : 0.9} roughness={0.15} />
      </mesh>
...
export default function WindowScene({ variant }: { variant: HomepageSceneVariant }) {
  ...
  return (
    <>
      <WindowView
        variant={variant}
        windowMeshRef={windowMeshRef}
        windowCenterX={windowCenterX}
        windowCenterY={windowCenterY}
      />
      <WindowFrame variant={variant} windowCenterX={windowCenterX} windowCenterY={windowCenterY} />
    </>
  );
}
