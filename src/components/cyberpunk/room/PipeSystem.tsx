...
function HorizontalPipes({ variant }: { variant: HomepageSceneVariant }) {
  return (
    <>
      {ROOM_OBJECTS.ceilingPipes.map((pipe, i) => (
        <mesh key={`hpipe-${i}`} position={pipe.position} rotation={[0, 0, Math.PI / 2]}>
          ...
          <meshStandardMaterial
            color={variant === 'day' ? '#8b97a6' : '#1a1a2a'}
            metalness={variant === 'day' ? 0.35 : 0.85}
            roughness={0.25}
          />
        </mesh>
      ))}
...
function VerticalPipes({ variant }: { variant: HomepageSceneVariant }) {
  return (
    <>
      {ROOM_OBJECTS.depthPipes.map((pipe, i) => (
        <mesh key={`vpipe-${i}`} position={pipe.position} rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry args={[pipe.radius, pipe.radius, pipe.length, 8]} />
          <meshStandardMaterial
            color={variant === 'day' ? '#94a3b8' : '#15152a'}
            metalness={variant === 'day' ? 0.35 : 0.85}
            roughness={0.25}
          />
        </mesh>
...
function CableTrays({ variant }: { variant: HomepageSceneVariant }) {
  return (
    <>
      {ROOM_OBJECTS.cableTrays.map((tray, i) => (
        <mesh key={`tray-${i}`} position={tray.position}>
          ...
          <meshStandardMaterial
            color={variant === 'day' ? '#9aa6b5' : '#1a1a2a'}
            metalness={variant === 'day' ? 0.35 : 0.8}
            roughness={0.3}
          />
        </mesh>
      ))}
...
export default function PipeSystem({ variant }: { variant: HomepageSceneVariant }) {
  return (
    <group>
      {/* 绠＄嚎鍜屾ˉ鏋跺睘浜庣粨鏋勫眰锛岀洿鎺ュ鐢ㄥ竷灞€鏁版嵁锛屼究浜庣粺涓€瀵归綈鍜岀淮鎶?*/}
      <HorizontalPipes variant={variant} />
      <VerticalPipes variant={variant} />
      <CableTrays variant={variant} />
    </group>
  );
}
