'use client';

import { useDeployStatus } from '@/hooks/useDeployStatus';

/**
 * 全局部署状态浮动指示器
 * 仅在部署进行中显示，部署完成后自动隐藏
 */
export default function DeployStatusIndicator() {
  const { status, version, commit } = useDeployStatus();

  if (status !== 'deploying') {
    return null;
  }

  return (
    <div
      className="fixed bottom-6 right-6 z-50 flex items-center gap-3 overflow-hidden rounded-full border border-fuchsia-400/70 bg-slate-950/82 px-4 py-3 font-mono text-fuchsia-100 backdrop-blur-md shadow-[0_0_24px_rgba(217,70,239,0.45)] animate-[deployPulse_1.8s_ease-in-out_infinite]"
      title={`Deploying ${version || commit}`}
    >
      {/* 扫描线光带 */}
      <span className="pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,transparent,rgba(255,255,255,0.12),transparent)] animate-[deploySweep_2.4s_linear_infinite]" />

      {/* 脉冲圆点 */}
      <span className="relative h-2 w-2 rounded-full bg-fuchsia-300 shadow-[0_0_16px_rgba(217,70,239,0.95)]" />

      {/* 旋转齿轮 */}
      <svg
        aria-hidden="true"
        className="relative h-5 w-5 animate-spin"
        fill="none"
        viewBox="0 0 24 24"
      >
        <path
          d="M12 3v3m0 12v3m6.36-15.36-2.12 2.12M7.76 16.24l-2.12 2.12M21 12h-3M6 12H3m15.36 6.36-2.12-2.12M7.76 7.76 5.64 5.64"
          stroke="currentColor"
          strokeLinecap="round"
          strokeWidth="1.8"
        />
        <circle cx="12" cy="12" r="4" stroke="currentColor" strokeWidth="1.8" />
      </svg>

      {/* DEPLOYING 文字 + 扫描线 */}
      <span className="relative overflow-hidden text-xs font-bold tracking-[0.24em]">
        DEPLOYING
        <span className="absolute inset-x-0 top-0 h-px bg-cyan-200/80 animate-[deployScan_1.1s_linear_infinite]" />
      </span>

      {/* 版本号 */}
      {(version || commit) && (
        <span className="relative max-w-24 truncate text-[10px] tracking-[0.12em] text-cyan-100/68">
          {version || commit}
        </span>
      )}
    </div>
  );
}
