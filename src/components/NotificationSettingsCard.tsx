...
interface Props { hasMail: boolean; }
...
export default function NotificationSettingsCard({ hasMail }: Props) {
  ...
  return <Card loading={loading}>
    <Space orientation="vertical" size="middle" className="w-full">
      ...
      {!hasMail ? <Alert type="info" showIcon title="尚未填写邮箱" description="你仍会收到站内通知；填写邮箱后可开启邮件提醒。" /> : null}
      {rows.map((row) => <div key={row.key} className="rounded-md border border-slate-200 p-3 dark:border-slate-700">
        ...
        <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 dark:border-slate-800"><span className="text-sm"><MailOutlined className="mr-2" />邮件提醒</span><Switch size="small" checked={settings.email[row.key]} disabled={!hasMail || !settings.inbox[row.key]} onChange={(checked) => void save({ ...settings, email: { ...settings.email, [row.key]: checked } })} /></div>
      </div>)}
    </Space>
  </Card>;
}
