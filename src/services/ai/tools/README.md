...
| `prompt-template-tools.ts` | Prompt list/load schema 与 Agent scope 策略实例 |
...
| `list_prompts` / `load_prompt_template` | 是 | 是 | 是 |
| `web_search` | 是 | 是 | 是 |
| `read_source_url` | 否 | 否 | 是 |
| `get_current_draft` / `emit_draft_patch` | 否 | 是 | 否 |
| `search_topics` / `get_current_topic` / `emit_topic_patch` | 否 | 否 | 是 |

Prompt 工具共用一份 schema 和执行定义，但按 Agent 绑定静态 scope：Chat 可见 `system/chat`，Create 可见 `system/content/create_agent`，Topic 可见 `system/content/topic_agent`。加载时必须同时满足模板和版本为 `ACTIVE`、scope 获准。
...
- system prompt 中的 `@slug` 必须先从原始模板解析，再渲染运行时草稿、选题和用户上下文，业务内容不能改变 Prompt 绑定。
