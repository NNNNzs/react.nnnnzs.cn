...
5. **关键模型补强**：减少 primitive 方块感，优先补交互和叙事最强的家具。
6. **移动端交互验证**：书架/服务器 click 展开卡片、外部点击关闭、显示器点击等数据家具交互在移动端需要验证和降级处理。
7. **组件重命名/抽象**：把 `CyberpunkBanner` 逐步抽象为 `Homepage3DBanner`，保留 `variant="day" | "night"`。
8. **静态 fallback**：生成一张高质量静态海报，用于低端设备和 reduced motion 降级。
