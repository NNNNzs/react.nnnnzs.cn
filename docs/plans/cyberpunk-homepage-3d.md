...
- [ ] 调整 `HomePageClient.tsx` 列表区外壳，增加首屏到文章流的渐隐过渡、section 背景、标题 HUD 和加载按钮文案。
- [ ] 调整 `PostListItem.tsx`，强化日志面板、角标、卡片挂线、终端指标和 hover 发光。
- [ ] 为移动端保留单列阅读体验，时间线收敛到左侧或隐藏，只保留 `LOG` 编号与卡片氛围。
- [ ] 校验日间主题：日间不强行赛博化，可保留更轻的纸张/档案流样式；夜间启用完整日志流。
