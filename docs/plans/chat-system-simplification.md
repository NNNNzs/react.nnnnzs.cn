...
```mermaid
flowchart TD
    A["用户问题"] --> B["第 1 次 LLM 调用<br/>思考 + 决定调用工具"]
    B --> C["执行工具<br/>向量搜索"]
    C --> D["第 2 次 LLM 调用<br/>基于结果生成答案"]
    D --> E["返回答案"]
```
...
```mermaid
flowchart TD
    A["用户问题"] --> B["向量检索<br/>无 LLM"]
    B --> C["注入上下文"]
    C --> D["1 次 LLM 调用<br/>直接生成答案"]
    D --> E["返回答案"]
```
...
flowchart TB
    ...
    subgraph Services["服务层"]
        subgraph Search["检索服务层 vector-search"]
            C1["向量搜索"]
            C2["结果排序"]
        end

        subgraph LLM["LLM 服务层 ai/llm"]
            D1["提示词构建"]
            D2["流式生成"]
        end
    end
...
```mermaid
flowchart TD
    A["[1] 用户发送消息"] --> B["[2] 前端验证<br/>非空、长度限制"]
    B --> C["[3] 发送到 API<br/>POST /api/chat"]
    C --> D["[4] API 验证参数"]
    D --> E["[5] 向量检索<br/>检索相关文章"]
    E --> F["[6] 构建提示词<br/>注入检索到的上下文"]
    F --> G["[7] 调用 LLM<br/>流式生成"]
    G --> H["[8] SSE 流式返回答案"]
    H --> I["[9] 前端渲染<br/>打字机效果"]
```
...
```mermaid
flowchart TD
    A["[1] 用户问题预处理"] --> A1["移除特殊字符"]
    A --> A2["提取关键词"]
    A1 --> B
    A2 --> B

    B["[2] 查询向量化"] --> B1["使用 Embedding 模型"]
    B --> B2["生成查询向量"]
    B1 --> C
    B2 --> C

    C["[3] 相似度搜索"] --> C1["Qdrant 向量搜索"]
    C --> C2["Top-K 结果 K=5"]
    C --> C3["相似度阈值过滤 >0.7"]
    C1 --> D
    C2 --> D
    ...
    D["[4] 结果去重和排序"] --> D1["按文章 ID 去重"]
    D --> D2["按最高分排序"]
    D1 --> E
    D2 --> E

    E["[5] 内容提取"] --> E1["获取文章详细信息"]
    E --> E2["提取相关片段"]
    E1 --> F
    E2 --> F
...
```mermaid
graph TB
    subgraph RAGLayout["简单 RAG 布局"]
        A1["📚 参考文章 (3)"]
        A2["└─ 📄 日本之旅 ⭐ 0.95"]
        A3["└─ 📄 韩国攻略 ⭐ 0.87"]
        A4["└─ 📄 东南亚日记 ⭐ 0.82"]
        B1["根据知识库中的文章，作者去过日本、"]
        B2["韩国等多个国家旅游。其中日本之旅的"]
        B3["相关性最高（0.95），文章中详细描述"]
        B4["了东京、大阪等地的旅行经历..."]
    end
```
