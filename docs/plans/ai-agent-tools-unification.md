...
- [x] 为 Prompt Skill、文章元数据、文章全文和请求级资源工具补充最小特征测试。
- [x] Chat 保留 Prompt Skill 探索能力，但仅允许 `system/chat` scope。
...
- [x] `buildTopicTools` 只组合共享工具和 Topic 请求级工具，并补齐 Skill list/load 能力。
...
- [x] mention 解析和 Skill 加载统一校验 `ACTIVE + skill + Agent scope/allowlist`。
- [x] Chat、Create、Topic 的 Prompt Skill 可见范围与实际工具白名单保持一致。
- [x] 未授权、ARCHIVED、非 skill 模板返回稳定且可观测的拒绝结果。
...
- [x] Topic Agent 已装配 `list_prompt_skills` / `load_prompt_skill_template`。
- [x] Chat Agent 的 Skill 工具只允许 `system/chat` scope。
- [x] 草稿、选题、用户资料或文章内容中的 `@slug` 不会改变 Agent 的 Skill 绑定。
- [x] ARCHIVED、非 skill 或未授权 scope 模板不能通过 Skill 工具加载。
