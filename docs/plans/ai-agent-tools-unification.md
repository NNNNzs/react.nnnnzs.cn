...
> 状态：📝 计划中
> 创建时间：2026-07-14
> 涉及范围：Chat Agent、Create Agent、Topic Agent、AI Template Prompt Skills
...
4. 后续新增文章查询、网页搜索和 Prompt Skill 工具时，一部分直接复用旧业务工具，一部分在各 Agent 文件中重新声明 LangChain wrapper，逐渐形成多套定义。
...
`list_prompt_skills` 和 `load_prompt_skill_template` 已在 `src/services/ai/tools/langchain-tools.ts` 中定义为 LangChain `StructuredTool`，Create Agent 又通过 `wrapListPromptSkills` / `wrapLoadPromptSkillTemplate` 重新包装一次。两套代码分别维护 Zod schema、description、错误格式和返回序列化，容易产生行为漂移。
...
Topic Agent 的 system prompt 和 `compilePromptTemplate` 生成的 metadata 会提示调用 `load_prompt_skill_template`，但当前 `buildTopicTools` 没有装配该工具。工具已经在其他文件中定义，并不等于 Topic Agent 可以调用它。

### 2.4 无状态工具和请求级工具没有明确边界

文章搜索、文章全文读取、Prompt Skill 加载等工具不依赖当前请求状态，适合定义为可复用单例。
...
### 2.5 Prompt Skill 能力边界缺少统一约束

工具装配、模板 `@slug` 绑定、模板状态和 scope 当前没有形成同一套授权规则：

- Agent 是否拿到 `list_prompt_skills` / `load_prompt_skill_template` 由工具数组决定；
- Agent 启动时看到哪些 Skill metadata 由 system prompt 中的 `@slug` 决定；
- 加载接口目前仍需补齐 `ACTIVE + skill + Agent scope` 校验；
...
5. Agent 工具清单成为明确的能力白名单，并与 system prompt、Prompt Skill scope 保持一致。
...
flowchart TB
    subgraph Domain[业务服务层]
        ...
        PostContent[文章全文读取]
        PromptTemplate[Prompt Skill 查询与加载]
    ...
    subgraph SharedTools[共享 LangChain 工具定义]
        ...
        ListSkillsTool[list_prompt_skills]
        LoadSkillTool[load_prompt_skill_template]
...
├── prompt-template-tools.ts      # list_prompt_skills / load_prompt_skill_template
...
- `list_prompt_skills`：列出当前 Agent 可发现的 Skill metadata。
- `load_prompt_skill_template`：加载当前 Agent 已获授权的 Skill 正文。
...
| `list_prompt_skills` | 按 scope 决定 | 是 | 可选 |
| `load_prompt_skill_template` | 按 scope 决定 | 是 | 是 |
...
- [ ] 枚举所有 `tool()`、自定义 `Tool`、`build*Tools` 和 `bindTools()` 调用点。
- [ ] 生成当前工具矩阵，记录每个 Agent 实际绑定的 name、schema 和 description。
- [ ] 为 Prompt Skill、文章元数据、文章全文和请求级资源工具补充最小特征测试。
- [ ] 确认 Chat 是否保留 Prompt Skill 探索能力，以及 Skill scope 的最终规则。

### 阶段 1：建立共享 LangChain 工具定义

- [ ] 新建共享 `article-tools.ts`，迁移 `search_posts`、`search_articles` 和 `get_post_content`。
- [ ] 统一 `search_posts_meta` / `search_posts` 的模型侧名称、参数和返回结构。
- [ ] 新建 `prompt-template-tools.ts`，只保留一份 list/load schema、description 和执行逻辑。
- [ ] 新建 `web-tools.ts`，统一 Create / Topic 的 Tavily 搜索实现和默认参数。
- [ ] 提取统一的成功/失败 JSON 序列化帮助函数，避免每个 wrapper 自行处理。

### 阶段 2：收敛请求级工具工厂

- [ ] 将 `get_draft` 明确命名为 `get_current_draft`，继续隐藏 draft ID。
- [ ] 将 `get_topic` 明确命名为 `get_current_topic`，继续隐藏 topic ID。
- [ ] 工厂上下文包含 actor / scope，但不向模型 schema 暴露身份参数。
- [ ] 检查 `getContentDraft`、`getContentTopic` 及写入辅助函数的数据范围，补齐必要的服务层授权入口。
- [ ] 保持 `emit_*_patch` confirm-first 和 `returnDirect` 行为不变。

### 阶段 3：简化 Agent 装配

- [ ] `chatTools` 只引用共享工具单例。
- [ ] `buildCreateTools` 只组合共享工具和 Create 请求级工具。
- [ ] `buildTopicTools` 只组合共享工具和 Topic 请求级工具，并补齐已在 prompt 中声明的 Skill 加载能力。
- [ ] 三个 Agent 的 `model.bindTools(tools)` 与 `createReactAgent({ tools })` 使用同一个数组实例。
- [ ] 增加重复工具名检测，禁止同一 Agent 装配两个同名不同 schema 的工具。

### 阶段 4：清理旧接口和目录

- [ ] 删除已经没有直接消费者的重复 wrapper。
- [ ] 评估自定义 `Tool` / `ToolResult` 是否仍有非 LangChain 消费者；无消费者时删除，有消费者时退化为纯业务函数返回类型。
- [ ] 把跨 Agent 工具移出 `create-tools` 目录，修正 import 和注释。
- [ ] 更新 `docs/designs/ai/create-agent.md`、`topic-agent.md`、`ai-lab.md` 中的工具清单和能力边界。

### 阶段 5：收紧 Prompt Skill 与动态上下文边界

- [ ] 从原始 system prompt 模板解析 `@slug`，再执行 mustache 渲染，避免运行时业务数据注入 mentions。
- [ ] mention 解析和 Skill 加载统一校验 `ACTIVE + skill + Agent scope/allowlist`。
- [ ] Chat、Create、Topic 的 Prompt Skill 可见范围与实际工具白名单保持一致。
- [ ] 未授权、ARCHIVED、非 skill 模板必须返回稳定且可观测的拒绝结果。

## 八、风险与控制

### 8.1 工具名称变化影响模型行为

将 `search_posts_meta` 收敛为 `search_posts`、将 `get_draft` 改为 `get_current_draft` 会影响 system prompt、调试日志和历史测试。迁移时应先提供兼容别名或一次性同步所有 prompt，再删除旧名。
...
按 Prompt Skill → 文章工具 → Web 工具 → 请求级工厂的顺序小步迁移。每阶段保持类型检查、lint 和 Agent 冒烟测试通过，不同时重写底层查询业务。

## 九、验证清单

- [ ] `pnpm typecheck` 通过。
- [ ] `pnpm lint` 通过。
- [ ] Chat、Create、Topic 实际绑定工具快照与批准后的能力矩阵一致。
- [ ] 每个共享工具的 name、description 和 Zod schema 在源码中只有一个定义来源。
- [ ] Create 与 Topic 共用同一个 `search_posts` 和 `get_post_content` 工具对象。
- [ ] Topic Agent 可以按已绑定的 `@slug` 调用 `load_prompt_skill_template`。
- [ ] Chat Agent 无法加载其 scope / allowlist 之外的创作 Skill。
- [ ] 草稿、选题、用户资料或文章内容中的 `@slug` 不会改变 Agent 的 Skill 绑定。
- [ ] ARCHIVED 或非 skill 模板不能通过 Skill 工具加载。
- [ ] `get_current_draft` 无模型参数，只能读取当前路由绑定的草稿。
- [ ] 如新增 `get_draft_by_id`，跨用户或超出数据范围的 ID 必须被拒绝。
- [ ] `emit_draft_patch` / `emit_topic_patch` 仍只发送待确认建议，不直接写数据库。
- [ ] LangGraph 调试日志中不存在同一 Agent 的重复工具名。
