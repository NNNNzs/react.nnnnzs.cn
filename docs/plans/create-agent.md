...
- [x] `src/services/ai/tools/create-tools/`：`list_prompt_skills`、`load_prompt_skill_template`、`get_draft`、`search_posts`、`get_post_content`、`web_search`、`generate_image`、`poll_image_job`、`emit_draft_patch`；`buildCreateTools` 闭包工厂；`draft-patch.ts` 共享类型
...
1. 草稿创建时保存 `topicSnapshot` 和 `templateSnapshot`，后续修改选题或模板不回写历史草稿。
2. 草稿 Agent 每次运行时主动加载快照，把选题作为受控参考上下文注入，不依赖用户再次粘贴。
3. 创建草稿不自动调用模型；草稿页提供“AI 根据选题生成初稿”按钮，由用户显式触发已有 chat/SSE 流程。
4. `xhs/note` 继续使用图文工作台；`zhihu/article` 复用现有 `MarkdownEditor` 编辑和预览 Markdown。
5. 小红书和知乎分别使用平台模板与结构化输出校验，继续共用 `DraftPatch`、差异确认和保存语义。
...
7. 从同一个选题分别创建小红书和知乎草稿，验证平台映射、选题/模板快照和 Topic 状态更新
