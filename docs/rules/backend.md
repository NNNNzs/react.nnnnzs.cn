...
### API 错误响应规范
**所有 API 路由必须使用统一的 errorResponse DTO**，禁止手动构建响应对象：

```typescript
// ✅ 正确：使用 errorResponse DTO
import { successResponse, errorResponse } from '@/dto/response.dto';
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json(successResponse({ message: '操作成功' }));
}
...
### 数据库操作规范
本项目使用 Prisma 作为 ORM，使用 **`prisma db push`** 同步数据库结构。
...
### 概述
项目提供统一的客户端 IP 地址获取工具，支持多种代理和 CDN 环境。
...
### 支持的代理头部
按优先级依次检查：
1. `x-forwarded-for` - 代理服务器（取第一个 IP）
2. `x-real-ip` - Nginx 等
3. `cf-connecting-ip` - Cloudflare

### IP 格式验证
工具会自动验证 IP 格式（IPv4/IPv6），无效的 IP 会被跳过。

### 环境差异处理
- **开发环境**：无法获取 IP 时返回 `127.0.0.1` 并记录警告日志
- **生产环境**：无法获取 IP 时抛出错误

```typescript
// 开发环境警告
⚠️ 无法获取客户端 IP，使用开发环境固定值。生产环境请确保配置正确的代理头部。

// 生产环境错误
Error: 无法获取客户端真实 IP 地址，请检查代理服务器配置（x-forwarded-for、x-real-ip 或 cf-connecting-ip）
```
...
export async function canLike(targetType: string, targetId: number, request: NextRequest) {
  const ip = getClientIp(request);
