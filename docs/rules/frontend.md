...
#### 腾讯云 CDN 图片压缩规范 ⚠️ IMPORTANT

项目使用 **腾讯云数据万象图片处理** 对 CDN 图片进行按比例/尺寸压缩，文档参考：
[`https://cloud.tencent.com/document/product/460/46823`](https://cloud.tencent.com/document/product/460/46823)。

统一使用 `src/lib/image.ts` 中的工具函数：
...
export enum ImageOptimizationType {
  POST_LIST_COVER = 'post_list_cover',     // 文章列表封面（600px 宽）
  POST_CARD_COVER = 'post_card_cover',     // 卡片封面（400px 宽）
  COLLECTION_COVER = 'collection_cover',   // 合集封面（600px 宽）
  SMALL_THUMBNAIL = 'small_thumbnail',     // 小缩略图（200px 宽）
  NONE = 'none',
}
...
export function optimizeImageUrl(
  ...
  // 仅处理 https://static.nnnnzs.cn 域名的图片
  // 已包含 imageMogr2 参数的 URL 不会重复追加
}
```
...
1. **仅对 CDN 域名生效**
   - 只处理 `https://static.nnnnzs.cn` 开头的图片 URL
   - 其他域名（占位图、第三方图片等）保持原样，不添加任何参数
...
2. **必须关闭 Next.js 自带优化**
   ...
   <Image
     ...
     alt={post.title || ''}
     unoptimized={true}                 // 关键：关闭 Next.js 内置优化，避免与腾讯云冲突
     fill
...
3. **典型场景对应优化类型**
   - 文章列表封面：`ImageOptimizationType.POST_LIST_COVER`
   - 文章卡片封面：`ImageOptimizationType.POST_CARD_COVER`
   - 合集封面：`ImageOptimizationType.COLLECTION_COVER`
   - 小尺寸封面/后台表格缩略图：`ImageOptimizationType.SMALL_THUMBNAIL`

4. **不使用 CDN 压缩的场景**
   - 所有头像（Avatar）图片
   - `Banner` 横幅背景图（使用 `backgroundImage`，并且不做压缩）
...
**重要**：项目性能优化遵循 **Vercel React Best Practices** 规范。

### 使用方式

开发过程中遇到性能相关问题时，使用以下命令：

```bash
/vercel-react-best-practices
```

该 skill 包含 57 条规则，涵盖：
- 消除瀑布请求（CRITICAL）
- 包体积优化（CRITICAL）
- 服务端性能（HIGH）
- 客户端数据获取（MEDIUM-HIGH）
...
### 已优化的组件
1. `BackToTop` - useCallback + passive 事件监听
2. `PostListItem` - React.memo
3. `CollectionCard` - React.memo
4. `CollectionSelector` - React.memo + useCallback
5. 文章管理页 - resize 节流
