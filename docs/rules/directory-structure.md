...
│   │   ├── oauth/                # OAuth 2.0 端点
│   │   ├── comment/              # 评论 API（新增）
│   │   ├── entity-change/        # 实体变更日志 API（新增）
│   │   ├── upload/               # 文件上传 API（新增）
│   │   ├── fs/                   # 文件系统 API（新增）
│   │   ├── collections/          # 合集管理 API（新增）
│   │   └── github/               # GitHub 集成
...
│   ├── status/                   # 状态页面（新增）
│   ├── register/                 # 注册页面（新增）
│   ├── authorize/                # OAuth 授权页面（新增）
│   ├── introspect/               # Token 检查端点（新增）
│   ├── revoke/                   # Token 撤销端点（新增）
│   ├── c/                        # 管理后台
│   │   ├── tools/                # 工具页面（新增）
│   │   ├── vector-search/        # 向量搜索管理（新增）
│   │   ├── collections/          # 合集管理
│   │   ├── edit/[id]/            # 文章编辑器
│   │   ├── post/                 # 文章管理
│   │   ├── user/                 # 用户管理
...
│   ├── auth.ts                   # 认证服务
│   ├── comment.ts                # 评论服务（新增）
│   ├── entity-change-detector.ts # 变更检测服务（新增）
│   ├── entity-change-log.ts      # 变更日志服务（新增）
