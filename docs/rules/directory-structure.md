...
- 主题切换、灯光布局和场景约定优先写到 `sceneLayout.ts` 与 `theme.ts`，不要散落在各个模型文件里。
- 功能性灯具必须把“可见灯体”和“真实光源”封装在同一个组件里一起创建，避免只改模型不改灯源或只改灯源不改模型。
- 只有纯装饰的发光贴图、HUD、指示点才允许是 mesh-only；凡是承担照明语义的灯条、吊灯、台灯、招牌灯都要使用同组件封装。
- 灯具组件优先按 `*Fixture` / `*Light` / `*Lamp` 这类语义命名，组件内部统一负责位置、模型、发光材质和 `pointLight` / `spotLight`。
