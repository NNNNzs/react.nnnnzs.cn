...
| Prompts | `/c/ai-lab/prompts` | 系统级 Prompt 模板、版本、diff、`@` 引用和 MCP 暴露管理 |
...
系统级 Prompt 模板库。模板主表保存稳定 `slug`、作用域、别名、description、MCP 暴露 metadata 和当前激活版本；版本表保存 Markdown 正文、checksum、变更说明和版本 metadata。`description` 是运行时供 Agent 选择 Prompt 的摘要，必须直接说明“何时加载、用于什么写作风格”，而不是写复杂能力矩阵。
...
`@slug` 是系统 Prompt 正文里的**模板绑定语法**。三个 Agent（Create / Topic / Chat）构建 system prompt 时统一走「先解析原始模板的 Prompt 绑定，再渲染 mustache 运行时变量」两步：

1. `compilePromptTemplate(rawTemplate, agentPolicy)` 用 `MENTION_PATTERN` 只扫描数据库系统模板原文里的 `@slug`，并校验 `ACTIVE + Agent scope`；
2. `createMustachePromptTemplate(compiled.content).format({...})` 再填入 `{{draftTitle}}` / `{{runtimeContext}}` 等运行时变量。

这样草稿、选题、文章或用户资料中的 `@slug` 无法注入新的 Prompt。模板提及决定启动时显示的 Prompt metadata，Agent scope 白名单决定 `list_prompts` 和 `load_prompt_template` 最终能发现、加载哪些 Prompt；两层必须同时满足。`load_prompt_template` 会拒绝 `DRAFT`、`ARCHIVED` 或不在当前 Agent scope 的模板与版本。

`type` 已统一为 `prompt`，系统提示词、工作流入口和按需加载的风格指南都使用同一模板模型；是否启动时注入由调用链决定，是否可被 Agent 发现由 scope 白名单决定，是否暴露给 MCP 由 `metadata_json.mcpExposed` 决定。

`scope` 固定取值为 `system`（全站通用）、`chat`、`create_agent`、`topic_agent`、`content`（小红书/知乎等平台通用）。平台相关的风格或方法论 Prompt 统一归 `content`，不要散落到 `system`。
...
- `list_prompts` 只返回 metadata，`load_prompt_template` 按 slug 加载完整正文。
- Prompt 工具按 Agent scope 装配：Chat=`system/chat`，Create=`system/content/create_agent`，Topic=`system/content/topic_agent`。
- MCP 只注册 `metadata_json.mcpExposed === true` 且模板与当前版本均为 `ACTIVE` 的标准 Prompt；MCP 与 Agent 工具复用同一套状态和 scope 校验服务。
- Create Agent 以草稿 `type` 和 Prompt `description` 选择风格指南：图文使用小红书风格，长文使用知乎 Markdown 风格。
- 系统 Prompt、按需加载 Prompt 正文与业务页面上下文分离：模板在 AI Lab 管理，页面实时上下文由业务入口在调用时传入。
...
- 新增或修改按需 Prompt 时，必须维护清晰的 `description`，说明加载条件和输出风格，确保 Agent 可以只靠 metadata 做按需选择。
