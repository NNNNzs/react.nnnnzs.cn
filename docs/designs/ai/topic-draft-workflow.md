...
5. 草稿 Agent 当前主要依靠 `get_draft` 和模型自行选择模板，没有稳定、可审计的“选题快照 + 平台模板 + 当前草稿”运行时上下文契约。
...
| 工具工厂 | `buildTopicTools({ topicId?, userId, emitPatch })` |
| 核心工具 | `search_topics`、`get_topic`、`search_posts`、`get_post_content`、`web_search`、`read_source_url`、`emit_topic_patch` |
