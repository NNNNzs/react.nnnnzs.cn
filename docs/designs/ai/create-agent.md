...
核心交互：用户说「把这篇扩展成 3 张小红书图卡」→ 助手先读取可用 Prompt 的 metadata，再按需加载风格指南 → 产出正文或图卡提示词计划 → 提交结构化建议 → 用户确认应用 → 用户在页面手动提交图片生成。
...
sequenceDiagram
    ...
    Agent->>Agent: list_prompts() → metadata
    Agent->>Agent: load_prompt_template() → style guide
...
创作前，Agent 调用 `list_prompts` 获取 ACTIVE Prompt 的摘要 metadata，并依据当前草稿 `type` 与 Prompt `description` 选择需要的风格指南；仅选中的 Prompt 才通过 `load_prompt_template` 加载完整正文。`note` 应加载小红书图文风格指南，`article` 应加载知乎 Markdown 长文风格指南。该规则不依赖复杂的平台/任务能力矩阵。
...
| `src/services/ai-template/index.ts` | 系统级 Prompt 模板服务 |
...
| `src/services/ai/tools/prompt-template-tools.ts` | Prompt 工具定义与 Agent scope 白名单 |
...
| 工具 | 类型 | 作用 |
|------|------|------|
| `list_prompts` | 只读 | 查询可用 Prompt 模板 metadata |
| `load_prompt_template` | 只读 | 按 slug 读取完整 Prompt 模板正文 |
...
### 6.2 看到 `✓ load_prompt_template`、`✓ search_posts`、`✓ emit_draft_patch` 代表成功了吗？

代表对应 LangChain Tool 已完成调用，基本可以判断 Agent 的工具链路跑通了。

- `load_prompt_template` 成功：Agent 已按 slug 加载完整 Prompt 模板正文，例如小红书风格指南。
...
- **选题转草稿**：保存选题快照和草稿类型；创作时由 Agent 按草稿类型选择所需写作风格 Prompt，详见[选题完善与多平台草稿转换](./topic-draft-workflow.md)
