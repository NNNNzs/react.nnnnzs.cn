...
**实际实施情况**：
1. ✅ 阶段 1：文本清理逻辑已修复
   - `src/services/embedding/text-splitter.ts:213-246` 已更新
   - 保留代码块标记 `[语言块]`
   - 保留行内代码内容

2. ✅ 阶段 2：简化的向量化服务已创建
   - `src/services/embedding/simple-embedder.ts` 已实现
   - 全量更新逻辑完整

3. ✅ 阶段 3：已集成异步队列系统
   - 使用 `queueEmbedPost` 代替直接调用
   - 不阻塞主流程，提高用户体验

4. ⚠️ 实施方案调整
   - 原计划：同步执行向量化，失败回滚
   - 实际方案：异步队列系统 + 错误日志
   - 理由：避免阻塞 API 响应，提高系统可用性
