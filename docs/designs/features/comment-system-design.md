...
> **本文档定位**: 技术设计文档 - 说明评论系统的设计原理和实现方案
>
> **开发实施规范**详见:
> - [前端开发规范](../../rules/frontend.md) - 前端组件实现
> - [后端开发规范](../../rules/backend.md) - API 路由实现
> - [数据库开发规范](../../rules/database.md) - 数据模型和查询优化
...
- 他人在文章下评论时，通知文章作者；回复他人评论时，通知被回复的评论作者。评论者本人不会收到自己的通知。
- 站内通知是主收件箱，邮件是用户开启后的额外提醒；通知链接统一跳转文章页，不使用评论锚点。
- 邮件中的文章链接携带一次签名通知凭证，打开链接后会将对应通知标记为已读，再跳转文章页。
- 邮件模板可渲染隐藏的 `readTrackingPixelUrl` 图片以在加载远程图片时标记已读；用户或邮件客户端禁止加载远程图片时，不应将“打开邮件”视为已读。
- 退订邮件提醒须进入确认页并由用户显式确认；站内通知不受退订影响，完整设置在个人设置页管理。
